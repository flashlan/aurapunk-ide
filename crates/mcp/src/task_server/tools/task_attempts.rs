...
struct McpWorkspaceRepoInput {
    ...
    repo_id: Uuid,
    #[schemars(
        description = "Base branch for the worktree — MUST be an existing branch in the repo (e.g. 'main'). If it does not exist, the server falls back to the repo's default branch, then 'main'/'master'. Prefer 'main' unless you have confirmed another branch exists."
    )]
    branch: String,
}
...
struct StartWorkspaceRequest {
    ...
    #[schemars(
        description = "The coding agent executor to run ('CLAUDE_CODE', 'AMP', 'GEMINI', 'CODEX', 'OPENCODE', 'CURSOR_AGENT', 'QWEN_CODE', 'COPILOT', 'DROID', 'COMMAND_CODE')"
    )]
    executor: String,
...
struct StartWorkspaceResponse {
    workspace_id: String,
    #[schemars(
        description = "Session ID of the started session — pass to run_session_prompt / list_sessions."
    )]
    session_id: String,
    #[schemars(
        description = "Execution ID of the first (kickoff) execution started by this call — pass to get_execution / list_pending_approvals to monitor it."
    )]
    execution_id: String,
}
