...
impl ExecutionProcess {
    ...
    /// Find the latest running coding-agent execution process for a session.
    ///
    /// Used to route a headed follow-up into an already-live interactive session
    /// (the caller still confirms the action is interactive and its tmux session
    /// is alive) instead of spawning a second `--resume` agent.
    pub async fn find_latest_running_coding_agent_for_session(
        pool: &SqlitePool,
        session_id: Uuid,
