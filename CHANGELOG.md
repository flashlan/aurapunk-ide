...
- MCP tools to manage a project's card statuses (board columns):
  `list_project_statuses`, `create_project_status`, `update_project_status`,
  `delete_project_status`, and `inject_sdlc_statuses` (the SDLC preset).
  Backed by new `/api/project-statuses` envelope CRUD endpoints, and shared
  with the Settings → Card Statuses section through a single backend preset.

## [0.3.2] - 2026-09-14

### Added

- Official download links for detected coding agents and graphical editors in
  the first-run wizard.
- A version-pinned `npx` launcher for Cloud IDE containers, with the compiled
  server retained as the default fallback.

### Fixed

- Hosted Mem0 now starts disabled until the authenticated cloud account hands
  off its gateway URL and device-scoped token, avoiding startup errors.
